...
@_flask_app.route("/get_save_path")
@login_required
def get_save_path():
    """Get current save path"""
    return jsonify({"save_path": _application.save_path})


@_flask_app.route("/set_save_path", methods=["POST"])
@login_required
def set_save_path():
    """Set new save path with validation"""
    new_path = request.args.get("path")
    if not new_path:
        return jsonify({"code": "0", "message": "Path is empty"})
    if not os.path.exists(new_path):
        return jsonify({"code": "0", "message": "Directory does not exist"})
    if not os.path.isdir(new_path):
        return jsonify({"code": "0", "message": "Path is not a valid directory"})
    if not os.access(new_path, os.W_OK):
        return jsonify({"code": "0", "message": "Directory is not writable"})
    _application.save_path = new_path
    _application.config["save_path"] = new_path
    _application.update_config(immediate=True)
    return jsonify({"code": "1", "save_path": _application.save_path})
